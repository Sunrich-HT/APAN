...
def count_atom_typing(path):
    ...
    for filename in path:
        ...
        for context in result:
            mol = Chem.MolFromSmiles(context[-1])
            atoms = mol.GetAtoms()
...
def do_pair_encoding(args, path, fold, filename, set_name):
    ...
    smi = []
    adj_list = []
    all_files = Chem.SDMolSupplier(str("/home/jovyan/qm8/data_index/qm8.sdf"), False, False, False)
    dij_matrix = []
    for context in result:
        if args.task in ['qm8']:
            mol = all_files[context[1]]
        else:
            mol = Chem.MolFromSmiles(context[-1])
        if args.task in ['qm7']:
            xyz = eval(context[1])
        ...
        if atoms_num <= 100:
            for i, atom in enumerate(mol.GetAtoms()):
                ...
                if args.task in ['qm8']:
                    xyz = list(m.GetConformer().GetAtomPosition(i))
                    coord_list.append(xyz)
            coord_list = coord_list + (max_atoms_nums - len(coord_list)) * [[0, 0, 0]]
            dij = pairwise_distances(np.array(coord_list), np.array(coord_list))
            ...
            if args.task in ['qm7']:
                ...
                dij_matrix.append(dij)
                label = [float(x) if x != '' else None for x in context[2:]]
            elif args.task in ['qm8']:
                label = [float(x) if x != '' else None for x in context[2:]]
            else:
                label = [float(x) if x != '' else None for x in context[-2]]
            Y.append(label)
